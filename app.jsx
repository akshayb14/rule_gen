...
function Topbar({ screen, state, setState }) {
  const crumb = {
    ...
    review: <>Rules › <b>Review</b> {state.pendingRule && <span className="mono"> · {state.pendingRule.id}</span>}</>,
...
function App() {
  ...
  const screens = {
    ...
    review: <Review state={state} setState={setState} goto={goto} toast={toast} />,
